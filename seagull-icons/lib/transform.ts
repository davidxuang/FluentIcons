...
export function parseTransform(yargs: Argv) {
  return yargs
    .string('config')
    .string('in')
    .string('in-override')
...
export default function fun(
  argv: Partial<ReturnType<typeof parseTransform> extends Argv<infer P> ? P : never> = {},
) {
  const CONFIG = argv.config ?? './transform.toml';
  const SRC_DIR = argv.in ?? './obj/mono/resizable';
  const SRC_OVERRIDES = argv['in-override'] ?? [
    './obj/mono/override',
    './override/seagull/cmpt',
    './override/mono/20',
    './override/seagull/common',
  ];
  const DEST_DIR = argv.out ?? './obj/composed/seagull';
