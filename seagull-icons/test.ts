...
const argv = yargs().string('in').demandOption('in').strict().parseSync(hideBin(process.argv));
...
fs.readdirSync(argv.in, { recursive: true }).forEach((f) => {
  ...
  parser.parseString(fs.readFileSync(file), (err, doc: Doc) => {
    ...
    if (Math.max(...margin) >= tolerance) {
      const extremum = Math.max(...margin);
      console.warn(
        styleText(
          extremum >= tolerance * 2 ? 'yellow' : 'gray',
          `[${extremum.toFixed(4)}] ${file}: [${margin.map((m) => m.toFixed(4)).join(', ')}]`,
        ),
      );
    }
